...
## Table of Contents
  * [Features](#features)
    + [Arbitrary Messaging](#arbitrary-messaging)
    + [Lossy/Lossless Streaming](#lossylossless-streaming)
...
### Lossy/Lossless Streaming
Protofish supports both lossy and lossless binary streaming. Once a message notifying an open stream is sent, an independent logical stream is initiated. It allows a binary data to be sent through itself. For example, a lossy binary stream can be utilized to send live audio.
...
> **NOTE** Distinguishing between the concepts of a message and a payload may be confusing. It can be understood intuitively by thinking the payload as a product, and the message as some kind of cardboard box that encloses the product before shipping, together with a ZIP code.
...
Protofish entirely relies on UTP. Therefore, strict prerequisites are applied to UTP. Following list is the prerequisites.
...
- **Multiplexing** It MUST follow these conditions.
    ...
    - It MUST have a way to distinguish streams by their IDs, regardless of the reliability of a stream.
...
#### States
Protofish has multiple types of states. This state model is for a conceptual representation of Protofish. Therefore, it's not necessary to exactly follow the model as far as the implementations support the described functionality.
...
- **ServerHello** A packet that responds to the ClientHello
...
- **ArbitraryData** A packet to pass any downstream binary message
...
### Message
All payloads going through PMC MUST be serialized with the `Message` specification in [message.proto](protos/payload/message.proto). Field `context_id` indicates a context ID, therefore it MUST be unique. Using a simple atomic counter to resolve a next context ID is RECOMMENDED.
...
### Behavior
A server with an accidentally closed connection MAY store the client handle with **CLSD** state for a desired time, which is RECOMMENDED to be 15 minutes. A client with an accidentally closed connection MAY try to reconnect and resume. A resume operation MUST be done by setting a field `resume_connection_token` to the connection token acquired by `ServerHello` in a previous connection. A server SHOULD revert the **CLSD** state of a resumed client to **OPEN**, and continue the transmission.
...
## Error Handling
Both sides SHOULD send `Error` message to notify the peer that an error has been occurred. Also, `Close` message MAY followed to close the connection due to a critical error.
